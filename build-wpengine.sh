...
(
	...
	# Check for uncommitted changes in public_html, and refuse to proceed if there are any
	echo "Checking for untracked or changed files…"
	if [ -n "$(git ls-files public_html --exclude-standard --others)" ]; then
		echo -e "${RED}You have untracked files, please remove or commit them before building:${NC}"
		git ls-files . --exclude-standard --others
		exit 3
	fi
	if ! git -c core.fileMode=false diff --quiet --exit-code public_html; then
